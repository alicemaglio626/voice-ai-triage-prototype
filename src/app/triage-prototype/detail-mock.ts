...
// Why the call landed in triage — mirrors prod's triage reasons. Ops Escalation
// (a reviewer flagged it) is the common case; the QA reasons come from the
// QA-review lane.
...
export function reasonFor(id: string): TriageReason {
  const n = hash(id) % 10;
  if (n <= 5) return "Ops Escalation";
  if (n <= 8) return "QA divergence";
  return "QA inconclusive";
}

// A distinct soft fill per reason so the three read apart at a glance (same
